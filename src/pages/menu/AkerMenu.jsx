...
const AkerMenu = () => {
  ...
  const [AkerMenuData, setAkerMenuData] = useState({});
        ...
        <main className="product-area">
          {categories.map((category) => (
            <div key={category} ref={(el) => (sectionRefs.current[category] = el)}>
              {selectedCategory === category && AkerMenuData[category] && (
                <MenuSection title={category} products={AkerMenuData[category]} onProductClick={handleProductClick} />
              )}
            </div>
          ))}
        </main>
